"""
Re-scrape missing Semester 1 USNs
...
def rescrape_missing_sem1(url, max_workers=8):
    """Re-scrape only the missing semester 1 USNs"""
    
    missing_usns = get_missing_usns_from_db(1)
    
    if not missing_usns:
        print("❌ No missing USNs found to scrape.")
        return
    
    print("="*80)
    print(f"🔄 RE-SCRAPING MISSING SEMESTER 1 RESULTS")
    print("="*80)
    print(f"📋 Auto-detected {len(missing_usns)} missing USNs from database")
    
    if len(missing_usns) == 0:
        print("\n✅ No missing USNs! All students have semester 1 results.")
        return
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_usn = {
            executor.submit(get_vtu_results, usn, url, 1): usn 
            for usn in missing_usns
        }
    ...
    if error_count > 0:
        print("\n💡 Tip: Some USNs might be invalid or not have semester 1 results.")
        print("   You can manually check these USNs on VTU results website.")

if __name__ == "__main__":
    print("="*80)
    print("🚀 RE-SCRAPE MISSING SEMESTER 1 USNs")
    print("="*80)
    print()
    
    url = input("Enter VTU Semester 1 results URL: ").strip()
    ...
    missing_count = len(get_missing_usns_from_db(1))
    
    if missing_count == 0:
        print("✅ No missing USNs found! All students have semester 1 results.")
        import sys
        sys.exit(0)
    ...
    if confirm in ['yes', 'y']:
        rescrape_missing_sem1(url, max_workers=workers)
    else:
        print("❌ Cancelled")
